...
import { SessionType } from '../../scheduling/enums/session-type.enum';
...
export class EvaluationService {
  ...
  async sendSessionEvaluation(
    ...
    const deadline = new Date(
      Date.UTC(year, month - 1, day + 7, 23, 59, 59, 999),
    );
  ...
  async getSessionEvaluation(
    ...
    if (!session) {
      throw new NotFoundException({
        ...
        description:
          'No existe evaluación para esta sesión o no tiene permisos para verla',
      });
    }
    ...
    for (const answer of answers) {
      ...
      if (!evaluationsByStudent.has(key)) {
        ...
        const studentName =
          answer.studentParticipateSession?.student?.user?.name ??
          'Desconocido';
    ...
    const evaluationDetails = Array.from(evaluationsByStudent.values()).map(
      (evalData) => {
    ...
    const filteredEvaluations =
      studentId && userRole !== UserRole.TUTOR
    ...
    const finalEvaluations = filteredEvaluations.map((evaluation) => ({
      ...
      studentName:
        userRole === UserRole.TUTOR ? undefined : evaluation.studentName,
    }));
  ...
  async getTutorEvaluations(
    ...
    const evaluationsList = Array.from(sessionsByEval.values()).map(
      (sessionData) => {
    ...
    const ratingsByAspect = {
      clarity: allAspectRatings.get('CLARITY')?.length
        ? Number(
            (
              allAspectRatings.get('CLARITY')!.reduce((a, b) => a + b, 0) /
              allAspectRatings.get('CLARITY')!.length
            ).toFixed(2),
          )
        : 0,
      patience: allAspectRatings.get('PATIENCE')?.length
        ? Number(
            (
              allAspectRatings.get('PATIENCE')!.reduce((a, b) => a + b, 0) /
              allAspectRatings.get('PATIENCE')!.length
            ).toFixed(2),
          )
        : 0,
      punctuality: allAspectRatings.get('PUNCTUALITY')?.length
        ? Number(
            (
              allAspectRatings.get('PUNCTUALITY')!.reduce((a, b) => a + b, 0) /
              allAspectRatings.get('PUNCTUALITY')!.length
            ).toFixed(2),
          )
        : 0,
      knowledge: allAspectRatings.get('KNOWLEDGE')?.length
        ? Number(
            (
              allAspectRatings.get('KNOWLEDGE')!.reduce((a, b) => a + b, 0) /
              allAspectRatings.get('KNOWLEDGE')!.length
            ).toFixed(2),
          )
        : 0,
    };
    ...
    return {
      ...
      summary: {
        ...
        ratingsByAspect,
        averageBySubject:
          Object.keys(averageBySubject).length > 0
            ? averageBySubject
            : undefined,
      },
  ...
  async getStudentEvaluations(
    ...
    const uniqueEvaluationIds = new Set(
      filteredAnswers.map((a) => a.evaluationId),
    );
    const totalRecords = uniqueEvaluationIds.size;
    ...
    let evaluationsList = Array.from(evaluationsByEvalId.values()).map(
      (evalData) => {
        const ratingValues = Array.from(evalData.ratings.values());
        const overallRating =
          ratingValues.length > 0
    ...
    evaluationsList = evaluationsList.sort(
      (a, b) =>
  ...
  async getTutorMetrics(
    ...
    const sessionsBySubjectMap: Map<
      string,
      { subjectName: string; count: number; subjectId: string }
    > = new Map();

    for (const session of sessions) {
      // Type counting (GROUP = collaborative)
      if (session.type === SessionType.INDIVIDUAL) {
      ...
      if (session.modality) {
        if (
          session.modality.toUpperCase() === 'PRESENCIAL' ||
          session.modality.toUpperCase() === 'PRES'
        ) {
          sessionsByModalityMap.presencial++;
        } else if (
          session.modality.toUpperCase() === 'VIRTUAL' ||
          session.modality.toUpperCase() === 'VIRT'
        ) {
    ...
    const sessionsBySubject = Array.from(sessionsBySubjectMap.values()).map(
      (item) => ({
    ...
    for (const participation of participations) {
      if (participation.status === ParticipationStatus.ATTENDED) {
        presentCount++;
      } else if (participation.status === ParticipationStatus.ABSENT) {
        absentCount++;
      } else if (participation.status === ParticipationStatus.LATE) {
        lateCount++;
      }
    }
    ...
    const sessionsByMonthMap: Map<
      string,
      { sessions: number; ratingSum: number; ratingCount: number }
    > = new Map();

    for (const session of sessions) {
      const monthKey = new Date(session.scheduledDate)
        .toISOString()
        .substring(0, 7); // YYYY-MM

      if (!sessionsByMonthMap.has(monthKey)) {
        sessionsByMonthMap.set(monthKey, {
          sessions: 0,
          ratingSum: 0,
          ratingCount: 0,
        });
      ...
      const sessionAnswers = answers.filter(
        (a) => a.idSession === session.idSession,
      );
  ...
  private calculateAspectAverage(
