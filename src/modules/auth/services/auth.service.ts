...
export class AuthService {
    ...
    async registerStudent(registerDto: RegisterStudentDto,): Promise<RegisterResponse> {
        ...
        try {
            ...
            this.logger.log(`Usuario registrado exitosamente: ${savedUser.idUser}`);

            //Generación del token de confirmación
            const confirmationToken = await this.createEmailConfirmationToken(savedUser.idUser);

            //Envío del correo de confirmación
            await this.notificationService.sendEmailConfirmation(savedUser.email, savedUser.name, confirmationToken);
    ...
    private async createEmailConfirmationToken(userId: number): Promise<string> {
        try {
            const token = uuidv4();
