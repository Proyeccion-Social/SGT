...
export class NotificationsController {
  ...
  async availabilityChanged(@Body() dto: AvailabilityChangedDto) {
    ...
    try {
      ...
      return {
        message:           'Notificaciones de cambio de disponibilidad enviadas exitosamente',
        tutorId:           dto.tutorId,
  ...
  async hoursLimitAlert(@Body() dto: HoursLimitAlertDto) {
    ...
    try {
      ...
      const alertLevel =
        dto.usagePercentage >= 100 ? '100_PERCENT' :
        dto.usagePercentage >= 95  ? '95_PERCENT'  :
                                     '80_PERCENT';
