...
describe('frontend URL validation', () => {
  it('accepts configured local, staging, production and project preview origins', () => {
    ...
    expect(isAllowedFrontendUrl('https://atlas-test.proysocial.org')).toBe(true);
  ...
  it('rejects phishing origins and URLs with paths', () => {
    expect(isAllowedFrontendUrl('https://atlas.proysocial.org.attacker.com')).toBe(
      false,
    );
    expect(isAllowedFrontendUrl('https://atlas.proysocial.org/login')).toBe(false);
  ...
  it('uses the requested origin, configured fallback, then local fallback', () => {
    ...
    expect(
      resolveFrontendUrl(configService, 'https://atlas-iok6ek85-proysocialuds-projects.vercel.app'),
    ...
    expect(resolveFrontendUrl({ get: jest.fn().mockReturnValue(undefined) } as any)).toBe(
      'http://localhost:3000',
    );
  });
});