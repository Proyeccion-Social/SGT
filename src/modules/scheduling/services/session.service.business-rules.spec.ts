...
describe('SessionService — Business Rules (Integration)', () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 2. LÍMITE DIARIO DE HORAS
  // ═══════════════════════════════════════════════════════════════════════════

  describe('2 — Límite diario de horas del tutor (4 h)', () => {
    /**
     * CASO: El tutor ya tiene 3.5h agendadas ese día.
     * Solicitud de 1h → 4.5h > 4h → falla en el check transaccional.
     */
    it('rechaza agendamiento cuando sumar la nueva sesión supera las 4 h diarias', async () => {
      qrManager.createQueryBuilder.mockReturnValueOnce(
        makeQb('getRawOne', { totalHours: '3.5' }),
      ); // daily hours → falla
    ...
    /**
     * CASO: El tutor ya tiene 3h agendadas ese día.
     * Solicitud de 1h → exactamente 4h → debe pasar.
     */
    it('permite agendamiento cuando la suma de horas diarias es exactamente 4 h', async () => {
      qrManager.createQueryBuilder
        .mockReturnValueOnce(makeQb('getRawOne', { totalHours: '3' })) // daily OK
        .mockReturnValueOnce(makeQb('getOne', null)) // confirmedInSlot
        .mockReturnValueOnce(makeQb('getCount', 0)); // pendingCount
    ...
    /**
     * CASO: Límite diario también aplica al confirmar una sesión pendiente.
     * El tutor ya tiene 4h SCHEDULED ese día → confirmar otra sesión excede el límite.
     */
    it('rechaza confirmación de sesión cuando confirmar excedería las 4 h diarias del tutor', async () => {
      ...
      // Sesión ya existente que ocupa 4h (09:00–13:00)
      const existingScheduled = makeSession({
      ...
      // QB para obtener sesión con lock
      qrManager.createQueryBuilder
    ...
    /**
     * CASO: proposeModification con nueva duración que excedería el límite diario.
     * validateDailyHoursLimit debe lanzar BadRequestException.
     */
    it('rechaza propuesta de modificación cuando la nueva duración supera el límite diario', async () => {
      validationService.validateDailyHoursLimit.mockRejectedValue(
        new BadRequestException('El tutor ha alcanzado su límite diario de 4h'),
      );
      ...
      await expect(
        service.proposeModification('student-1', 'session-1', {
          newDurationHours: 3, // superaría el límite
        }),
      ).rejects.toThrow(BadRequestException);
    });
  });
