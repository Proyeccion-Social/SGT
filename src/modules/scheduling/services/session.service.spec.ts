...
const makeQb = (terminalFn: 'getOne' | 'getMany' | 'getCount' | 'getManyAndCount', value: any) => {
  const qb: any = {
...
describe('SessionService', () => {
  ...
  const mockSession = (overrides: Partial<any> = {}): any => ({
    ...
    virtualLink: null,
    tutorConfirmed: true,
    studentParticipateSessions: [{ idStudent: 'student-1', status: ParticipationStatus.CONFIRMED, student: { idUser: 'student-1', user: { name: 'Alice' } } }],
  ...
  beforeEach(() => {
    ...
    availabilityService = {
      getAvailabilityById: jest.fn().mockResolvedValue({ startTime: '09:00', dayOfWeek: 0 }),
    };
    tutorService = {
  ...
  describe('rejectSession', () => {
    ...
    it('sets status to REJECTED_BY_TUTOR and deletes scheduled session', async () => {
      const session = mockSession({ status: SessionStatus.PENDING_TUTOR_CONFIRMATION });
      sessionRepository.findOne.mockResolvedValue(session);
      scheduledSessionRepository.delete.mockResolvedValue({ affected: 1 });

      const result = await service.rejectSession('tutor-1', 'session-1', { reason: 'busy' });

      expect(session.status).toBe(SessionStatus.REJECTED_BY_TUTOR);
      expect(session.rejectionReason).toBe('busy');
      expect(sessionRepository.save).toHaveBeenCalled();
      expect(scheduledSessionRepository.delete).toHaveBeenCalledWith({ idSession: 'session-1' });
  ...
  describe('cancelSession', () => {
    it('throws NotFoundException when session does not exist', async () => {
      ...
      await expect(
        service.cancelSession('student-1', 'session-1', { reason: 'plans changed' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('allows admin to cancel within 24h', async () => {
      ...
      const result = await service.cancelSession('admin-1', 'session-1', { reason: 'admin' });
    ...
    it('sets CANCELLED_BY_TUTOR when the tutor cancels', async () => {
      const session = mockSession({
        studentParticipateSessions: [{ idStudent: 'student-1', status: ParticipationStatus.CONFIRMED }],
      });
      sessionRepository.findOne.mockResolvedValue(session);
      scheduledSessionRepository.delete.mockResolvedValue({ affected: 1 });
  ...
  describe('proposeModification', () => {
    it('throws NotFoundException when session does not exist', async () => {
      ...
      await expect(
        service.proposeModification('student-1', 'session-1', { newScheduledDate: '2030-12-30' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ForbiddenException when user is neither participant nor tutor', async () => {
      ...
      await expect(
        service.proposeModification('unrelated-user', 'session-1', { newScheduledDate: '2030-12-30' }),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('creates modification request and sets session to PENDING_MODIFICATION', async () => {
      const session = mockSession();
      sessionRepository.findOne.mockResolvedValue(session);
      scheduledSessionRepository.findOne.mockResolvedValue({ idSession: 'session-1', idAvailability: 1 });
    ...
    it('sets modification request expiry to 1 day from now', async () => {
      sessionRepository.findOne.mockResolvedValue(mockSession());
      scheduledSessionRepository.findOne.mockResolvedValue({ idAvailability: 1 });

      const before = new Date();
      await service.proposeModification('student-1', 'session-1', { newScheduledDate: '2030-12-30' });
      const after = new Date();

      const savedRequest = modificationRequestRepository.save.mock.calls[0][0];
      const expiryMs = savedRequest.expiresAt.getTime();
      expect(expiryMs).toBeGreaterThanOrEqual(before.getTime() + 23 * 60 * 60 * 1000);
      expect(expiryMs).toBeLessThanOrEqual(after.getTime()  + 25 * 60 * 60 * 1000);
    });
  });
  ...
  describe('updateSessionDetails', () => {
    it('throws NotFoundException when session does not exist', async () => {
      ...
      await expect(
        service.updateSessionDetails('student-1', 'session-1', { title: 'New title' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ForbiddenException when user is neither participant nor tutor', async () => {
      ...
      await expect(
        service.updateSessionDetails('unrelated-user', 'session-1', { title: 'New title' }),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('throws BadRequestException when the session has already started', async () => {
      ...
      await expect(
        service.updateSessionDetails('student-1', 'session-1', { title: 'New' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('does not include unchanged fields in the change notification', async () => {
      ...
      await service.updateSessionDetails('student-1', 'session-1', { title: 'Same title' });

      const changes = notificationsService.sendSessionDetailsUpdate.mock.calls[0][1];
  ...
  describe('respondToModification', () => {
    ...
    it('throws NotFoundException when no pending modification request exists', async () => {
      managerMock.findOne
        .mockResolvedValueOnce(mockSession({ status: SessionStatus.PENDING_MODIFICATION })) // session
    ...
    it('throws ForbiddenException when user is neither participant nor tutor', async () => {
      managerMock.findOne
        .mockResolvedValueOnce(mockSession({ status: SessionStatus.PENDING_MODIFICATION }))
    ...
    it('marks request as EXPIRED and restores SCHEDULED status when request has expired', async () => {
      const session  = mockSession({ status: SessionStatus.PENDING_MODIFICATION });
      const request  = pendingRequest({ expiresAt: new Date(Date.now() - 1000) }); // expired
    ...
    it('rejects modification: sets REJECTED status and restores SCHEDULED session', async () => {
      ...
      const result = await service.respondToModification('tutor-1', 'session-1', false);
    ...
    it('accepts modification: applies new date and restores SCHEDULED status', async () => {
      ...
      const scheduledSession = { idAvailability: 1, scheduledDate: FUTURE_DATE };
      ...
      const result = await service.respondToModification('tutor-1', 'session-1', true);
  ...
  describe('confirmSession', () => {
    it('throws NotFoundException when session does not exist', async () => {
      managerMock.createQueryBuilder.mockReturnValueOnce(makeQb('getOne', null)); // session not found
    ...
    it('throws ForbiddenException when tutor does not own the session', async () => {
      managerMock.createQueryBuilder.mockReturnValueOnce(
        makeQb('getOne', mockSession({ status: SessionStatus.PENDING_TUTOR_CONFIRMATION })),
      );
    ...
    it('throws BadRequestException when the slot is already confirmed for another student', async () => {
      ...
      managerMock.findOne.mockResolvedValueOnce({ idAvailability: 1, scheduledDate: FUTURE_DATE });
    ...
    it('confirms session and auto-rejects zero competitors when slot is free', async () => {
      ...
      managerMock.findOne
        .mockResolvedValueOnce({ idAvailability: 1, scheduledDate: FUTURE_DATE }) // scheduledSession
    ...
    it('auto-rejects competing pending sessions when confirming', async () => {
      ...
      const competitor1 = {
        idSession: 'session-2',
        session: mockSession({ idSession: 'session-2', status: SessionStatus.PENDING_TUTOR_CONFIRMATION }),
      };
      const competitor2 = {
        idSession: 'session-3',
        session: mockSession({ idSession: 'session-3', status: SessionStatus.PENDING_TUTOR_CONFIRMATION }),
      };
      ...
      expect(notificationsService.sendSessionRejection).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('createIndividualSession', () => {
    ...
    it('runs all domain validations before acquiring locks', async () => {
      ...
      expect(validationService.validateStudentNotTutor).toHaveBeenCalledWith('student-1', 'tutor-1');
    ...
    it('creates session, scheduledSession, and participation on success', async () => {
      ...
      expect(managerMock.create).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ status: SessionStatus.PENDING_TUTOR_CONFIRMATION }),
      );
      expect(result.success).toBe(true);
      expect(queryRunnerMock.commitTransaction).toHaveBeenCalled();
    });
