...
const TUTORS_DATA = [
  {
    name: 'Carlos Ramírez',
    email: 'carlos.ramirez@udistrital.edu.co',
    phone: '3201234567',
  },
  {
    name: 'Ana Martínez',
    email: 'ana.martinez@udistrital.edu.co',
    phone: '3107654321',
  },
  {
    name: 'Luis Pérez',
    email: 'luis.perez@udistrital.edu.co',
    phone: '3159876543',
  },
  {
    name: 'María González',
    email: 'maria.gonzalez@udistrital.edu.co',
    phone: '3124567890',
  },
];
...
const STUDENTS_DATA = [
  {
    ...
    email: 'sebastian.torres@udistrital.edu.co',
    career: 'Ingeniería de Sistemas',
    modality: PreferredModality.PRES,
  },
  {
    name: 'Valentina López',
    email: 'valentina.lopez@udistrital.edu.co',
    career: 'Ingeniería Industrial',
    modality: PreferredModality.VIRT,
  },
  {
    name: 'Andrés Morales',
    email: 'andres.morales@udistrital.edu.co',
    career: 'Ingeniería Electrónica',
    modality: PreferredModality.PRES,
  },
  {
    name: 'Camila Ruiz',
    email: 'camila.ruiz@udistrital.edu.co',
    career: 'Ingeniería de Sistemas',
    modality: PreferredModality.VIRT,
  },
];
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    for (const s of SUBJECTS_DATA) {
      ...
      if (!subj) {
        subj = await subjectRepo.save(
          subjectRepo.create({ ...s, isActive: true }),
        );
    ...
    for (let day = 1; day <= 5; day++) {
      for (let hour = 7; hour <= 17; hour++) {
        ...
        let slot = await availRepo.findOne({
          where: { dayOfWeek: day, startTime },
        });
        if (!slot) {
          slot = await availRepo.save(
            availRepo.create({ dayOfWeek: day, startTime }),
          );
        }
    ...
    const QUESTIONS_DATA = [
      {
        content: '¿Qué tan claro fue el tutor al explicar los temas?',
        aspect: QuestionAspect.CLARITY,
        label: 'Claridad',
        displayOrder: 1,
      },
      {
        content: '¿El tutor mostró paciencia al resolver tus dudas?',
        aspect: QuestionAspect.PATIENCE,
        label: 'Paciencia',
        displayOrder: 2,
      },
      {
        content: '¿El tutor fue puntual al inicio y fin de la sesión?',
        aspect: QuestionAspect.PUNCTUALITY,
        label: 'Puntualidad',
        displayOrder: 3,
      },
      {
        content: '¿El tutor demostró dominio del tema tratado?',
    ...
    for (const td of TUTORS_DATA) {
      ...
      let tutorProfile = await tutorRepo.findOne({
        where: { idUser: tutorUser.idUser },
      });
      if (!tutorProfile) {
        tutorProfile = await tutorRepo.save(
          tutorRepo.create({
    ...
    for (const sd of STUDENTS_DATA) {
      ...
      let studentProfile = await studentRepo.findOne({
        where: { idUser: studentUser.idUser },
      });
      if (!studentProfile) {
        studentProfile = await studentRepo.save(
          studentRepo.create({
    ...
    for (const { tutorIdx, subjectIndices } of tutorSubjectMap) {
      ...
      for (const si of subjectIndices) {
        ...
        const exists = await tutorSubjectRepo.findOne({
          where: { idTutor, idSubject },
        });
        if (!exists) {
          await tutorSubjectRepo.save(
            tutorSubjectRepo.create({ idTutor, idSubject }),
          );
        }
      }
    }
    ...
    const studentSubjectRepo = dataSource.getRepository(
      StudentInterestedSubject,
    );
    const studentSubjectMap: Array<{
      studentIdx: number;
      subjectIndices: number[];
    }> = [
    ...
    for (const { studentIdx, subjectIndices } of studentSubjectMap) {
      ...
      for (const si of subjectIndices) {
        ...
        const exists = await studentSubjectRepo.findOne({
          where: { idStudent, idSubject },
        });
        if (!exists) {
          await studentSubjectRepo.save(
            studentSubjectRepo.create({ idStudent, idSubject }),
          );
        }
      }
    }
      ...
      for (const day of rule.days) {
        for (const hour of rule.hours) {
          ...
          const slot = availabilities.find(
            (a) =>
              a.dayOfWeek === day && a.startTime.startsWith(`${pad(hour)}:`),
          );
          if (!slot) continue;
    ...
    if (!session1) {
      session1 = await sessionRepo.save(
        sessionRepo.create({
          idTutor: s1Tutor,
          idSubject: s1Subject,
          ...
          description:
            'Sesión introductoria sobre límites, continuidad y derivadas básicas.',
      ...
      await scheduledSessionRepo.save(
        scheduledSessionRepo.create({
    ...
    if (!session2) {
      session2 = await sessionRepo.save(
        sessionRepo.create({
          idTutor: s2Tutor,
          idSubject: s2Subject,
          ...
          description:
            'Revisión de espacios vectoriales, bases ortonormales y transformaciones lineales.',
