...
const dataSource = new DataSource({
  ...
  entities: [
    ...
    StudentParticipateSession, Question, Answer,
    AppNotification, AuthSession, AuditLog, PasswordResetToken, EmailVerificationToken,
  ],
});
...
async function run(): Promise<void> {
  ...
  try {
    const sebastianUser = await userRepo.findOneOrFail({ where: { email: 'sebastian.torres@udistrital.edu.co' } });
    const sebastian     = await studentRepo.findOneOrFail({ where: { idUser: sebastianUser.idUser } });
    const carlosUser    = await userRepo.findOneOrFail({ where: { email: 'carlos.ramirez@udistrital.edu.co' } });
    const carlos        = await tutorRepo.findOneOrFail({ where: { idUser: carlosUser.idUser } });
    const subject       = await subjectRepo.findOneOrFail({ where: { name: 'Cálculo Diferencial' } });
    ...
    const session = await sessionRepo.save(sessionRepo.create({
      ...
      description:        'Revisión de límites, reglas de derivación y aplicaciones básicas.',
